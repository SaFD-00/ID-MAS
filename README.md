...
| [6] | Qwen3-4B | GPT-5.2 | Cross-Model | 대형→소형 전이 |
| [7] | Qwen3-8B | GPT-5.2 | Cross-Model | 대형→소형 전이 |
...
├── 1. ID-MAS Training          ← SFT 데이터 생성 (7개 실험 x 4개 데이터셋)
│   ├── 1.1 Self-Distillation   ← 실험 [1]~[5]
│   └── 1.2 Cross-Model         ← 실험 [6]~[7]
├── 2. LlamaFactory SFT         ← LoRA Fine-tuning & HF Upload
│   ├── 2.0 Setup               ← LlamaFactory 설치, 데이터/YAML 자동 생성
│   ├── 2.1 Self-Distillation   ← LoRA SFT [1]~[5]
│   ├── 2.2 Cross-Model         ← LoRA SFT [6]~[7]
│   └── 2.3 Merge & Upload      ← LoRA 병합 → HuggingFace Hub 업로드
└── 3. Evaluation               ← Baseline vs SFT 성능 비교
    ├── 3.1 Self-Distillation   ← 평가 [1]~[5]
    └── 3.2 Cross-Model         ← 평가 [6]~[7]
```
