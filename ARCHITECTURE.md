...
flowchart TB
    ...
    subgraph Learning["3-Phase 학습 단계"]
        subgraph Phase1["Phase 1: Scaffolding"]
            P1_IN[학습 문제] --> P1_TA[Task Analysis 생성]
            P1_TA --> P1_IR[Initial Response 생성]
            P1_IR --> P1_CMP{정답 비교}
            P1_CMP -->|정답| P1_OK[Phase 1 응답 사용]
            P1_CMP -->|오답| P2_IN
        end

        subgraph Phase2["Phase 2: Coaching"]
            P2_IN[Phase 1 오답 문제] --> P2_WO[Weak Objectives 분석]
            P2_WO --> P2_CDB[Coaching DB 생성]
            P2_CDB --> P2_FR[Fixed Response 생성]
            P2_FR --> P2_CMP{정답 비교}
            P2_CMP -->|정답| P2_OK[Phase 2 응답 사용]
            P2_CMP -->|오답| P3_IN
        end

        subgraph Phase3["Phase 3: Modeling"]
            P3_IN[Phase 2 오답 문제] --> P3_TM[Teacher Modeling Response]
            P3_TM --> P3_OK[Teacher 응답 사용]
        end
    end

    subgraph Output["SFT 데이터 생성"]
        P1_OK --> SFT[SFT Training Data]
        P2_OK --> SFT
        P3_OK --> SFT
    end
...
모든 학습 문제에 대해 Task Analysis와 함께 초기 응답을 생성합니다.
...
Phase 1에서 오답인 문제들에 대해 Coaching DB를 생성하고 수정된 응답을 생성합니다.

```
Phase 1 오답 문제
    ↓
[Weak Objectives 분석] ← 40% 이상 오류율 목표 식별
    ↓
[Coaching DB 생성] ← 교사 모델이 교정 가이드 생성
    ↓
[Fixed Response 생성] ← Coaching 적용
    ↓
[정답 비교] → 정답/오답 판정
```

### Phase 3: Modeling (교사 모델링)

Phase 2에서도 오답인 문제들에 대해 교사가 직접 모범 응답을 생성합니다.

```
Phase 2 오답 문제
    ↓
[Teacher Modeling Response] ← 교사 모델이 직접 응답 생성
    ↓
최종 SFT 데이터로 사용
```
...
[Phase 1: Scaffolding]
  ...
  - 정답 비교
    ↓
[Phase 2: Coaching] (오답만)
  - Weak Objectives 분석
  - Coaching DB 생성
  - Fixed Response
    ↓
[Phase 3: Modeling] (여전히 오답)
  - Teacher Modeling Response
    ↓
[SFT 데이터 생성]
    ↓
...
### SFT_ID-MAS
- ID-MAS 3-Phase Pipeline으로 생성된 SFT 데이터로 학습된 모델
...
└─────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 2: 3-Phase 학습 단계 (Learning Phase)                       │
│   - DomainLoader.load_training_data("gsm8k")                    │
│   - Phase 1: Scaffolding (Initial Response)                     │
│   - Phase 2: Coaching (오답에 대한 교정)                         │
│   - Phase 3: Modeling (여전히 오답인 경우 교사 모델링)            │
│                                                                 │
