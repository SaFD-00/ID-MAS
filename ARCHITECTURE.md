...
| **Step 5** | Final Solution | Teacher | `teacher_model.py` | Case C 최종 솔루션 생성 |
...
- **재구성 불필요**: Student의 PO 통과 응답을 SFT 데이터로 직접 사용
- **근거**: Student가 Scaffolding을 통해 스스로 PO를 충족한 응답이므로, 별도의 Teacher 재구성 없이 해당 응답이 SFT 학습에 적합
- **코드**: `nodes.py` — `sft_output = response`
...
- **변환 규칙**:
  ...
  - `output`: Case A → 학생 응답, Case B → PO 통과 학생 응답, Case C → Teacher 최종 풀이
...
| **B** | 2~5회차 PO 충족 | Student의 PO 통과 응답 직접 사용 | No |
...
| **Step 5** | Case C 생성 실패 | ground_truth 기반 응답 사용 (Case B는 재구성 불필요) |
...
| `SCAFFOLDING_ARTIFACT_PROMPT` | Teacher HOT/LOT Scaffolding 생성 (구조화된 마크다운 출력) |
| `STUDENT_FEEDBACK_RESPONSE_PROMPT` | Student 재응답 (전체 Scaffolding Artifact 참조) |
| `TEACHER_FINAL_SOLUTION_PROMPT` | Teacher Case C 최종 풀이 (평문 텍스트) |
