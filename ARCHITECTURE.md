...
- **동작**: `scaffolding_results`에서 Case A/B/C 항목을 SFT 형식으로 변환
- **변환 규칙**:
...
**ModelCache:**
- 글로벌 싱글톤으로 `(model_name, device, gpu_id)` 튜플 키로 관리
- Teacher/Student 동일 모델 사용 시 메모리 공유
- `gpu_id`가 지정되면 `RemoteLLMProxy`를 통해 subprocess에서 모델 로드
- `gpu_id`가 None이면 기존 인프로세스 vLLM 로드 (CUDA_VISIBLE_DEVICES 기반)
