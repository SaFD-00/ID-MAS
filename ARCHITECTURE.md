...
| Agent | 역할 | 모델 |
|-------|------|------|
| **Teacher** | 교수 설계, PO 평가, Scaffolding 생성, Positive Feedback, 응답 재구성 | OpenAI API (`gpt-5.2`) 또는 로컬 HuggingFace |
| **Student** | 초기 응답 생성, Scaffolding 참조 재응답, Self-Refinement | 로컬 HuggingFace (기본: `Qwen/Qwen3-1.7B`) |
...
| **Step 5a** | Positive Feedback | Teacher | `teacher_model.py` | Case A/B: PO 충족 시 강점 + 개선점 피드백 |
| **Step 5b** | Self-Refinement | Student | `student_model.py` | Case A/B: Positive Feedback 기반 응답 개선 |
| **Step 5c** | Final Solution | Teacher | `teacher_model.py` | Case C: 최종 솔루션 생성 |
...
│  │   [Step 5a] Teacher Positive Feedback               │   │
│  │       ↓                                              │   │
│  │   [Step 5b] Student Self-Refinement                  │   │
│  ├──────────────────────────────────────────────────────┤   │
│  │ Finalization                                         │   │
│  │   [Step 5c] Final Solution (Case C만)                │   │
│  │       ↓                                              │   │
...
- **동작**: 각 PO에 대해 `is_satisfied` 판단 + Student response 참조 feedback 작성
- **출력**:
  ```json
  {
...
- **동작**: `STUDENT_FEEDBACK_RESPONSE_PROMPT`에 전체 Scaffolding Artifact 텍스트를 포함하여 개선된 응답 생성
- **DB 참조 추출**: `StudentModel.extract_db_references()` — "Information Retrieved from Scaffolding Artifact:" 섹션 파싱

#### Step 5a: Positive Feedback (Teacher) — Case A/B

- **함수**: `TeacherModel.generate_positive_feedback()`
- **조건**: `all_satisfied == True` (Case A 또는 Case B)
- **동작**: 모든 PO가 충족된 Student 응답의 강점을 분석하고, 구체적인 개선 제안을 생성
- **출력**: `feedback_text` (강점 + 개선 제안 + 통합 가이드)
- **Fallback**: 최대 3회 재시도, 실패 시 PO evaluation의 satisfied feedback들을 조합하여 기본 피드백 생성

#### Step 5b: Self-Refinement (Student) — Case A/B

- **함수**: `StudentModel.self_refine_response()`
- **조건**: Step 5a에서 positive feedback이 생성된 경우
...
#### Step 5c: Final Solution (Teacher) — Case C

- **함수**: `TeacherModel.generate_final_solution()`
- **조건**: `iteration == max_iterations` AND `all_satisfied == False`
- **동작**: `extract_student_weaknesses()`로 약점 추출 → `last_iteration_summary` + `student_weaknesses`를 기반으로 교육적 정답 풀이를 평문 텍스트로 생성
...
- **변환 규칙**:
  - `instruction`: 원본 instruction + `SCAFFOLDING_SYSTEM_PROMPT` 동적 결합 (Task Analysis 포함)
  - `input`: `"Question: {문제 텍스트}"`
  - `output`: Case A → Student Self-Refined Response, Case B → Student Self-Refined Response, Case C → Teacher 최종 풀이
  - `metadata`: `id`, `sft_case`, `ground_truth`

### Case 분류

| Case | 조건 | SFT 응답 소스 | Self-Refinement | Step 5c 필요 |
|------|------|--------------|-----------------|-------------|
| **A** | 1회차 PO 충족 | Teacher Positive Feedback → Student Self-Refined Response | Yes (Step 5a→5b) | No |
| **B** | 2~5회차 PO 충족 | Teacher Positive Feedback → Student Self-Refined Response | Yes (Step 5a→5b) | No |
| **C** | 5회 후 PO 미충족 | Teacher가 정답 기반 재구성 | No | Yes |

**성공 조건**: 모든 Performance Objectives가 충족되면 (`all_satisfied == True`) 성공으로 처리됩니다.
**Self-Refinement**: Case A/B에서 PO 충족 후 Teacher의 Positive Feedback(강점 + 개선점)을 기반으로 Student가 응답을 self-refine하여 SFT 데이터 품질을 향상시킵니다.
...
| **Step 5a** | Positive Feedback 생성 실패 | PO evaluation의 satisfied feedback 조합으로 기본 피드백 생성 |
| **Step 5b** | Self-Refinement 실패 | 원본 Student 응답을 SFT output으로 사용 |
| **Step 5c** | Case C 생성 실패 | ground_truth 기반 응답 사용 |
...
| 상수 | 용도 |
|------|------|
| `SCAFFOLDING_SYSTEM_PROMPT` | Student 초기 응답 시스템 프롬프트 (Task Analysis 포함) |
| `TEACHER_INTERVENTION_SYSTEM_PROMPT` / `_USER_PROMPT` | Teacher PO 평가 (system: 역할 정의, user: 평가 데이터) |
| `SCAFFOLDING_ARTIFACT_SYSTEM_PROMPT` / `_USER_PROMPT` | Teacher HOT/LOT Scaffolding 생성 (system: 역할 정의, user: 입력 데이터) |
| `STUDENT_FEEDBACK_RESPONSE_PROMPT` | Student 재응답 (전체 Scaffolding Artifact 참조) |
| `TEACHER_POSITIVE_FEEDBACK_SYSTEM_PROMPT` / `_USER_PROMPT` | Teacher PO 충족 시 강점 + 개선점 피드백 생성 (Case A/B) |
| `STUDENT_SELF_REFINEMENT_PROMPT` | Student Positive Feedback 기반 응답 Self-Refinement (Case A/B) |
| `TEACHER_FINAL_SOLUTION_SYSTEM_PROMPT` / `_USER_PROMPT` | Teacher Case C 최종 풀이 (system: 역할 정의, user: 문제/이력 데이터) |
