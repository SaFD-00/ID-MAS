...
[입력]
  원본 데이터셋 (GSM8K, MATH, ReClor, ARC-c)
       │
       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  1단계: 추론 경로 생성 (src/reasoning/multi_model_reasoning_gen.py)         │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────┐    ┌──────────────────┐    ┌─────────────────────┐        │
│  │ 25개 시드   │───▶│ 3단계 추론 생성  │───▶│ vLLM 배치 추론      │        │
│  │ 모듈        │    │ (추상→구체)      │    │ (temperature=0.85)  │        │
│  └─────────────┘    └──────────────────┘    └─────────────────────┘        │
│                                                                             │
│  파라미터: temperature=0.85, top_p=0.9, max_tokens=2048, num_samples=20    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
       │
       │ 출력: data/generated/<MODEL_SHORT>/<DATASET>_generated.jsonl
       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  2단계: 데이터 필터링 (src/pipeline/filtering.py)                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐         │
│  │ 답변 추출       │───▶│ 정규화 처리     │───▶│ Exact Match     │         │
│  │ (boxed/####)    │    │ (숫자/옵션)     │    │ 필터링          │         │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘         │
│                                                                             │
│  정답 유형: numeric (수학), option (객관식 A/B/C/D)                         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
       │
       │ 출력: data/filtered/<MODEL_SHORT>/<DATASET>_filtered.json
       │       data/filtered/<MODEL_SHORT>/merged_train.json (병합)
       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  3단계: 모델 학습 (src/finetune_code/multi_model_finetune.py)                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌───────────────────────────────────────────────────────────────────┐     │
│  │ 3B-14B 모델: Full Fine-tuning                                     │     │
│  │ • 전체 가중치 업데이트                                            │     │
│  │ • GPU: 1-2개 (16-48GB)                                            │     │
│  └───────────────────────────────────────────────────────────────────┘     │
│                                                                             │
│  ┌───────────────────────────────────────────────────────────────────┐     │
│  │ 70B+ 모델: LoRA + 4-bit Quantization                              │     │
...
│  └───────────────────────────────────────────────────────────────────┘     │
│                                                                             │
│  하이퍼파라미터: lr=1e-6, epochs=3, batch=16, warmup=0.03, scheduler=cosine│
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
       │
       │ 출력: checkpoints/<MODEL_SHORT>/
       ▼
[출력]
  학습된 모델 체크포인트
```

### 25개 시드 추론 모듈 (REASONING_MODULES)

ReGenesis의 핵심은 25개의 다양한 추론 전략을 사용하여 추론 경로를 생성하는 것입니다:

| 카테고리 | 예시 모듈 |
|----------|----------|
| 실험적 접근 | "How could I devise an experiment to help solve that problem?" |
| 아이디어 생성 | "Make a list of ideas for solving this problem..." |
| 비판적 사고 | "Critical Thinking: This style involves analyzing..." |
| 시스템 사고 | "Use systems thinking: Consider the problem..." |
| 단계별 접근 | "Let's think step by step." |
| 역방향 사고 | "Let's work backwards from the goal..." |
| 비유적 사고 | "Think of this problem as if it were a puzzle..." |

전체 25개 모듈은 `src/reasoning/multi_model_reasoning_gen.py`의 `REASONING_MODULES` 변수에서 확인할 수 있습니다.

### 3단계 추론 경로 생성 (Abstract to Concrete)

```
┌─────────────────┐
│ 1. 일반 가이드  │  "How should I approach this type of problem?"
│    라인 생성    │  → 일반적인 문제 해결 전략 수립
└────────┬────────┘
         ▼
┌─────────────────┐
│ 2. 태스크 특화  │  "Given this specific problem, what structure should I use?"
│    구조 생성    │  → 문제 유형에 맞는 구체적 접근법 결정
└────────┬────────┘
         ▼
┌─────────────────┐
│ 3. 추론 경로   │  "Now solve the problem step by step."
│    생성        │  → 실제 답을 도출하는 추론 과정 생성
└─────────────────┘
```

이 방식은 기존 STaR의 직접 생성 방식과 달리, **추상적 전략에서 구체적 풀이로** 진행하여 OOD(Out-of-Domain) 일반화 성능을 약 10.7% 향상시킵니다.
